...
echo "Enter servername or IP address for the middleman server[space.astro.cz]"
read MIDDLEMAN_SERVER
if [[ -z $MIDDLEMAN_SERVER ]]; then
  MIDDLEMAN_SERVER="space.astro.cz"
fi
...
echo "Checking to see if we can login using public key authentication: ssh $MIDDLEMAN_USERNAME@$MIDDLEMAN_SERVER"
...
echo "You should send this information to Bolidozor network administrator:
